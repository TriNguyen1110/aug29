...
// MeshDriftBackground — user-supplied reference component ("Mesh drift", generated by the
// 21st.dev Shader Builder), used for item 15 (BOARD.tsv) to fill the dead side-gutter space
// on the list page at wide viewports. GLSL and the uniform-packing setup are kept verbatim
// per the dispatch instructions — only the mount wrapper (components/side-gutters.tsx) and
// one UNIFORMS value are adapted:
//
//   UNIFORMS.cursorEnabled: true -> false. The reference's cursor-reactive warp/ripple/twist
//   effects assume this shader IS the primary focal visual the user is looking at and moving
//   their mouse over. Here it plays a decorative side-gutter role behind body text the user
//   is actually reading — a mouse-follow effect in peripheral vision competes for attention
//   without adding anything, and disabling it also skips the pointermove/scroll listener setup
//   below entirely (the component's own `if (UNIFORMS.cursorEnabled)` guard), which is a real
//   reduction in per-frame + listener cost for a background element on a hackathon-grade build
//   running two WebGL contexts at once (this one + waves-background.tsx). All other UNIFORMS
//   values (colors, scale, drift, grain, etc.) are untouched from the supplied source.
...
const UNIFORMS = {
  ...
  colorCount: 4,
  scale: 1.300, intensity: 0.560, paramA: 0.670, warp: 0.192,
  detail: 2.016, contrast: 1.167, brightness: 0.000, saturation: 1.000,
  hue: 0.0000, vignette: 0.150, blur: 0.0072, grain: 0.098,
  seed: 5069.0, rotate: 2.7227, offsetX: 0.090, offsetY: 0.150, drift: 0.148,
  // Disabled for the side-gutter decorative role — see file header comment above.
  cursorEnabled: false, cursorEffect: 2.0, cursorStrength: 0.730, cursorRadius: 0.365,
  oklab: 0.0, timeScale: -1.373,
}
